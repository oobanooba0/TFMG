...
function distant_sounds.play_sound_panned(data)
  ...
  for _, player in pairs(players) do
    local player_sound_data = {
      ...
      min_distance = data.min_distance or 50, --minimum distance defines the distance at which volume can start to drop off
    }
...
function distant_sounds.play_panned_sound_for_player(data)
  ...
  --game.print(distance)
  --game.print("distance: "..distance_factor)
  --now we gotta calculate the panning factor

  local player_x = player_position.x
  local sound_x = sound_origin.x or sound_origin[1]

  local panning_factor = ((((sound_x - player_x)/(distance)) * (1 - (distance_factor^3))) * 0.5) + 0.5--panning factor is a value from 0 to 1 corresponding to left ear/right ear. The distance is used so that closer sources still get panned appropriately.
  ...
  --game.print("Pan: "..panning_factor)
  --game.print("distance^3: "..distance_factor^3)
  ...
  --game.print("L: "..left_volume)
  --game.print("R: "..right_volume)

  player.play_sound({ path = left_path, volume_modifier = math.min(left_volume,1)})
  player.play_sound({ path = right_path, volume_modifier = math.min(right_volume,1)})
