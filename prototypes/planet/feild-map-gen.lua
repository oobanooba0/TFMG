...
--empty space
  {
    ...
    name = "field_empty_space",
    expression = "1",
  },
--dust tiles
  {
    type = "noise-expression",
    name = "field_dust_flat",
    expression = "field_asteroid_spots > 0.5",
  },
  {
    type = "noise-expression",
    name = "field_dust_patchy",
    expression = "field_asteroid_spots > 1",
  },
  {
    type = "noise-expression",
    name = "field_dust_crests",
    expression = "field_asteroid_spots > 2",
  },
  {
    type = "noise-expression",
    name = "field_dust_lumpy",
    expression = "field_asteroid_spots > 3",
  },
  {
    type = "noise-expression",
    name = "field_dust_crater",
    expression = "(field_craters < 0.1) + (field_craters > 0.6) + (field_asteroid_spots < 0.5) == 0",
  },
...
--asteroid field
...

  {
    ...
    expression = "starting_spot_at_angle{\z
    	...
    	distance = 0,\z
    	radius = 100,\z
    	x_distortion =  (field_wobble_x * 15) + (field_wobble_tiny_x * 3),\z
    	y_distortion =  (field_wobble_y * 15) + (field_wobble_tiny_y * 3)\z
			}"
  },
  ...
  {--big asteroids
    type = "noise-expression",
    name = "field_asteroid_spots_big",
    expression = "field_asteroid_spot_noise{\z
			seed1 = 1000,\z
      spacing = 150,\z
      radius = 250,\z
      favorability = asteroid_voids(x,y) -0.4,\z
      distortion_x = (field_wobble_x) + (field_wobble_tiny_x * 0.4),\z
      distortion_y = (field_wobble_y) + (field_wobble_tiny_y * 0.4),\z
  ...
  {--small asteroids
    type = "noise-expression",
    name = "field_asteroid_spots_small",
    expression = "field_asteroid_spot_noise{\z
			seed1 = 1,\z
      spacing = 20,\z
      radius = 20,\z
      favorability = asteroid_voids(x,y)^2-0.05,\z
      distortion_x = field_wobble_tiny_x,\z
      distortion_y = field_wobble_tiny_y,\z
  ...
  { --base spot noise for asteroids
	  type = "noise-function",
	  name = "field_asteroid_spot_noise",
